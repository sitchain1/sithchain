...
const char * const BITCOIN_CONF_FILENAME = "sithchain.conf";
const char * const BITCOIN_PID_FILENAME = "sithchaind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sithchain" is a composite category enabling all Sithchain-related debug output
            if(ptrCategory->count(string("sithchain"))) {
...
#else
    const char* pszModule = "sithchain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SithchainCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SithchainCore
    // Mac: ~/Library/Application Support/SithchainCore
    // Unix: ~/.sithchaincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SithchainCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SithchainCore";
#else
    // Unix
    return pathRet / ".sithchaincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sithchain.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sithchain.conf
